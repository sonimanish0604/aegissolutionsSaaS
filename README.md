# Aegis SaaS Platform

This repository contains the core services and automation that underpin the Aegis ISO 20022 platform.

## Documentation

- [MT→MX sample payloads](services/aegis-iso20022-api/docs/mt-mx-samples.md)
- [SOC 2 compliance automation](docs/SOC2_COMPLIANCE.md)
- [Branch protection recommendations](docs/BRANCH_PROTECTION.md)
- [Security automation checklist](docs/SECURITY_AUTOMATION.md)
- [S3 audit log retention policy](docs/S3_AUDIT_LOG_RETENTION_POLICY.md)
- [Audit pipeline blueprint](docs/AUDIT_PIPELINE.md)
- [Audit infra Terraform blueprints](docs/AUDIT_INFRA_TERRAFORM.md)
- [Manifest verification procedure](docs/S3_MANIFEST_VERIFICATION.md)
- [Audit data access procedure](docs/AUDIT_DATA_ACCESS_PROCEDURE.md)
- [Release management & semantic versioning](docs/RELEASE_MANAGEMENT.md)
- [Batch translate API](services/aegis-iso20022-api/docs/batch-translate.md)
